...
void main() {
  ...
  test('chat identity survives reload and expired sessions are read-only', () async {
  ...
  test('multipart media is uploaded once then referenced by a shared URL', () async {
    final requests = <http.Request>[];
    ...
    final client = MockClient((request) async {
      ...
      if (request.url.path.endsWith('/uploads')) {
        ...
        expect(latin1.decode(request.bodyBytes), contains('filename="photo.png"'));
        expect(latin1.decode(request.bodyBytes), contains(latin1.decode(bytes)));
        return chatResponse({
    ...
    await http.runWithClient(() async {
      final api = ApiService();
      final attachment = await api.uploadChatMedia(bytes, filename: 'photo.png');
  ...
  test('20 MB boundary is accepted and larger uploads are rejected locally', () async {
    var uploads = 0;
    final client = MockClient((request) async {
      uploads++;
      return chatResponse({
    ...
    await http.runWithClient(() async {
      ...
      for (final bytes in [Uint8List(0), Uint8List(ApiService.maxChatMediaBytes + 1)]) {
        await expectLater(
