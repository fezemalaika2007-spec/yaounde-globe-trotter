...
def get_all_favorites() -> list:
    """Return all favorites across all users."""
    return _read_json(FAVORITES_FILE)


def get_favorites_for_user(username: str) -> list:
    """Return favorite destination names for *username*."""
...
def set_favorites_for_user(username: str, destinations: list) -> None:
    """Set the favorite destinations list for *username*."""
    favs = get_all_favorites()
    found = False
    for entry in favs:
        if entry.get("username") == username:
            ...
            break
    if not found:
        favs.append({"username": username, "destinations": destinations})
